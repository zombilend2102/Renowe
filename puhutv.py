...
<body>
    ...
    <div class="filmpaneldis">
        ...
        <div class="filmpanel" onclick="showBolumler('{{ loop.index }}')">
        ...
        var diziler = {
            ...
            "{{ loop.index }}": {
                ...
                "bolumler": [
                    ...
                    {"ad": "{{ ep.full_name }}", "link": "{{ ep.stream_url }}"} {% if not loop.last %},{% endif %}
                    {% endfor %}
                ]
...
'''
    ...
    for index, series in enumerate(data, start=1):
        ...
        for ep_index, ep in enumerate(series['episodes']):
            episodes_js += f'{{"ad": "{ep['full_name']}", "link": "{ep['stream_url']}"}}'
            if ep_index < len(series['episodes']) - 1:
                episodes_js += ","
        diziler_js += f'''
            "{index}": {{
                "resim": "{series['img']}",
                "bolumler": [{episodes_js}]
            }}'''
        if index < len(data):
            diziler_js += ","
